...
#define PACKET_SET_LIFT 0x30
#define PACKET_ENABLE_LIFT 0x31
#define PACKET_DISABLE_LIFT 0x32
#define PACKET_CALLIBRATION_LIFT 0x33
|
...
void handlePacket() {
    ...
    while (Serial.available() || (packet_state==PKT_WAIT_DATA && packet_len == 0)) {
...
void handlePacketData(uint8_t *buffer, uint8_t data_type, uint16_t data_len) {
    ...
    if (data_type == PACKET_GET_POSES) {
        ...
        uint16_t speed = buffer[0] | (buffer[1] << 8);
        uint16_t accel = buffer[2] | (buffer[3] << 8);

        int num_poses = data_len / 3;

        // Точное соответствие типов сигнатуре библиотеки Feetech/Waveshare
        u8  id_list[num_poses];
        s16 pos_list[num_poses];    // Позиция: знаковое s16
        u16 speed_list[num_poses];  // Скорость: БЕЗзнаковое u16
        u8  accel_list[num_poses];  // Ускорение: u8

        const uint8_t* ptr = &buffer[4]; 

        for (int i = 0; i < num_poses; i++) {
            uint8_t id = *ptr++;
            uint16_t pos = ptr[0] | (ptr[1] << 8);
            ptr += 2;

            id_list[i]    = (u8)id;
            pos_list[i]   = (s16)convertPos2Sts(id, pos); // Приведение к s16
            speed_list[i] = (u16)speed;                   // Приведение к u16
            accel_list[i] = (u8)accel;
        }

        // Передаем массивы — теперь типы совпадают идеально
        st.SyncWritePosEx(id_list, num_poses, pos_list, speed_list, accel_list);
    } else if ( data_type == PACKET_SET_LIFT ) {
        Serial2.println(*((uint16_t*)buffer));
    } else if ( data_type == PACKET_ENABLE_LIFT ) {
        Serial2.println("enable");
    } else if ( data_type == PACKET_DISABLE_LIFT ) {
        Serial2.println("disable");
    } else if ( data_type == PACKET_CALLIBRATION_LIFT ) {
        Serial2.println("callibration");
    }
}
...
void processCommand(String command) {
    ...
    // Подъёмник
    else if (cmdName == "SET_LIFT")         { handleSetLift(args); }
    else if (cmdName == "DISABLE_LIFT")     { Serial2.println("disable"); }
    else if (cmdName == "ENABLE_LIFT")      { Serial2.println("enable"); }
    else if (cmdName == "CALLIBRATE_LIFT")  { Serial2.println("callibration"); }
